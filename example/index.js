...
(function (api) {
  ...
  api.subscribe(state => {
    ...
    el.find('.configurator__track-scale-visibility').attr('checked', state.trackScaleVisibility);
  ...
  el.find('.configurator__interval-mode')
    .on('click', (event) => api.setIntervalMode(!event.target.hasAttribute('checked')));

  el.find('.configurator__marker-visibility')
    .on('click', (event) => api.setMarkerVisibility(!event.target.hasAttribute('checked')));

  el.find('.configurator__track-scale-visibility')
    .on('click', (event) => api.setTrackScaleVisibility(!event.target.hasAttribute('checked')));
