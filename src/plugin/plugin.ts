import { RangeSliderComponent, RANGE_SLIDER_ACTIONS } from 'components/range-slider';

function main(this: JQuery): PluginApi {
  const rootComponent = new RangeSliderComponent();
  rootComponent.attachToDocument(this.get(0));

  const documentStyles = getComputedStyle(document.documentElement);
  const thumbWidth =
    parseFloat(documentStyles.getPropertyValue('--rs-thumb-width')) *
    parseFloat(documentStyles.fontSize);
  const ratio = thumbWidth / rootComponent.view.nativeElement.offsetWidth;

  const initData: RangeSliderModelData = {
    root: {
      color: '#FF9933',
      isVertical: false,
      ratio,
    },
    input: {
      min: -3.5,
      max: 4,
      step: 0.5,
      values: [-1.5, 2.0],
    },
    track: {},
  };

  rootComponent.coldStart(initData);

  if (process.env.NODE_ENV === 'development') {
    console.log('logger attached!');

    rootComponent.model.subscribe(data => {
      console.log(data);
    });
  }

  return {
    setVertical(value) {
      rootComponent.model.dispatch({
        type: RANGE_SLIDER_ACTIONS.CHANGE_IS_VERTICAL,
        payload: value,
      });
    },
  };
}

export { main };
